...
* Update translations (ping wumpus on IRC) see [translation_process.md](https://github.com/lux/lux/blob/master/doc/translation_process.md#synchronising-translations).

* Update manpages, see [gen-manpages.sh](https://github.com/lux/lux/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
* Update hardcoded [seeds](/contrib/seeds/README.md), see [this pull request](https://github.com/lux/lux/pull/7415) for an example.
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/lux-core/lux-detached-sigs.git
    git clone https://github.com/devrandom/gitian-builder.git
    git clone https://github.com/lux/lux.git

### Lux maintainers/release engineers, update version in sources
...
### Fetch and create inputs: (first time, or when dependency versions change)
    ...
    pushd ./gitian-builder
    mkdir -p inputs
    wget -P inputs https://luxcore.org/cfields/osslsigncode-Backports-to-1.7.1.patch
...
- Detached signatures will then be committed to the [lux-detached-sigs](https://github.com/lux-core/lux-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
space *do not upload these to the lux.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the lux.org server
  into `/var/www/bin/lux-core-${VERSION}`
...
people without access to `lux.org` to download the binary distribution.
Also put it into the `optional_magnetlink:` slot in the YAML file for
lux.org (see below for lux.org update instructions).

- Update lux.org version

  - First, check to see if the Lux.org maintainers have prepared a
    release: https://github.com/lux-dot-org/lux.org/labels/Releases

      - If they have, it will have previously failed their Travis CI
        checks because the final release files weren't uploaded.
        Trigger a Travis CI rebuild---if it passes, merge.

  - If they have not prepared a release, follow the Lux.org release
    instructions: https://github.com/lux-dot-org/lux.org#release-notes
...
- Announce the release:
  ...
  - lux-dev and lux-core-dev mailing list

  - Lux Core announcements list https://luxcore.org/en/list/announcements/join/

  - luxcore.org blog post

  - Update title of #lux on Freenode IRC

  - Optionally twitter, reddit /r/Lux, ... but this will usually sort out itself

  - Notify BlueMatt so that he can start building [the PPAs](https://launchpad.net/~lux/+archive/ubuntu/lux)

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/lux/lux/releases/new) with a link to the archived release notes.
