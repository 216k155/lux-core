...
*Setup instructions for a Gitian build of lux Core using a Debian VM or physical system.*
...
- [Building lux Core](#building-lux-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-lux-host/README.md).
...
Clone the git repositories for lux and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/luxproject/lux --recursive
git clone https://github.com/luxproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/lux/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the lux repository under 'Fetch and create inputs' to install sources which require
...
To build lux Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lux repository.
...
For example:
```bash
URL=https://github.com/laanwj/lux.git
...
# if wanting to use a different version of eth-cpp-lux:
./bin/gbuild --commit lux=${COMMIT},cpp-eth-lux=${ETHCOMMIT} --url lux=${URL},cpp-eth-lux=${ETHURL} ../lux/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the lux git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lux/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/luxproject/lux-detached-sigs.git

BTCPATH=/some/root/path/lux
SIGPATH=/some/root/path/lux-detached-sigs

./bin/gbuild --url lux=${BTCPATH},signature=${SIGPATH} ../lux/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lux-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lux-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lux-osx-build.assert
```
...
[luxproject/gitian.sigs](https://github.com/luxproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@lux.org) and he will commit them.
